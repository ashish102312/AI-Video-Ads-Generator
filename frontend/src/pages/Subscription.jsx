            ...
            <div className="grid md:grid-cols-3 gap-8 items-center">
                {plans.map((plan, i) => (
                    <div key={i} className={`card relative p-8 flex flex-col ${plan.highlighted ? 'border-primary-500 shadow-xl shadow-primary-500/20 scale-105 z-10' : 'border-slate-200 dark:border-dark-border'}`}>
                        {plan.highlighted && (
                            <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-primary-600 text-white font-bold text-xs uppercase tracking-wider py-1 my-4 px-4 rounded-full flex items-center gap-1">
